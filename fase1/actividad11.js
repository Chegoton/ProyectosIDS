...
// Crear documentos y asignarles un ID único
const documentContent = files.map((file, index) => `${index + 1}\t${file}\n`).join('');
fs.writeFileSync(documentFile, documentContent); // Cambio aquí
...
files.forEach((file, index) => {
    ...
    words.forEach(word => {
        ...
        if (wordDictionary[lowercaseWord].files) {
            wordDictionary[lowercaseWord].files.add(index + 1); // Usar el ID del documento en lugar del nombre del archivo
        } else {
            wordDictionary[lowercaseWord].files = new Set([index + 1]);
        }
    });
    ...
    for (const [word, count] of Object.entries(wordCountInFile)) {
        ...
        postingContent[word].push({ file: index + 1, count }); // Usar el ID del documento en lugar del nombre del archivo
    }
